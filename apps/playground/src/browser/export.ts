 ...
 * into a `.env` the user can run locally with the published `@rail402/*` packages, so the flow they
...
export function buildTerminalExport(session: Session, config: SessionConfig): TerminalExport {
  ...
  const command = "npx @rail402/agent-helpers pay $DEMO_URL --budget 0.10";
