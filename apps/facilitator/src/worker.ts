...
export interface WorkerEnv extends Record<string, string | undefined> {
  ...
  async fetch(request: Request, env: WorkerEnv): Promise<Response> {
    try {
      if (!cached) {
        ...
        cached = { config, app: createApp({ config, startedAt: Date.now() }) };
      }

      // Fail loudly rather than serving a catalog that silently empties itself.
      const path = new URL(request.url).pathname;
      if (path.startsWith("/discovery/") && env.BAZAAR_EPHEMERAL_ACK !== "1") {
